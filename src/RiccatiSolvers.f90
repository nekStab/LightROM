   ...
   subroutine projector_splitting_DLRA_riccati_integrator_rdp(X, A, B, CT, Qc, Rinv, Tend, tau, info, &
      ...
      if (opts%if_rank_adaptive) then
         if (.not. X%rank_is_initialised) then
            ...
            call set_initial_rank(X, A, B, CT, Qc, Rinv, tau, opts%mode, exptA, trans, tol)
         end if
      end if
      ...
      dlra : do istep = 1, nsteps
         ...
         if (opts%if_rank_adaptive) then
            call rank_adaptive_projector_splitting_DLRA_step(X, A, B, CT, Qc, Rinv, tau, opts%mode, exptA, trans, info, rk_reduction_lock, tol)
         else
            call fixed_rank_projector_splitting_DLRA_step(X, A, B, CT, Qc, Rinv, tau, opts%mode, info, exptA, trans)
         end if
