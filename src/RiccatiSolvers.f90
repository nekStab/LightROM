module LightROM_RiccatiSolvers
   ...
   use LightROM_LyapunovSolvers, only : M_map
...
!      call log_settings(X, Tend, opts)
      call init_ricc(X, A, B, CT, Qc, Rinv, Tend, tau, exptA, opts, iftrans, options)
      ...
      dlra : do istep = 1, opts%nsteps
         ...
         if (opts%if_rank_adaptive) then
            call rank_adaptive_projector_splitting_DLRA_step(X, A, B, CT, Qc, Rinv, opts%tau, opts%mode, exptA, trans, info, rk_reduction_lock, opts%tol)
         else
            call fixed_rank_projector_splitting_DLRA_step(X, A, B, CT, Qc, Rinv, opts%tau, opts%mode, info, exptA, trans)
         end if
         ...
         if ( X%rk > 0 .and. (mod(istep, opts%chkstep) == 0 .or. istep == opts%nsteps) ) then
            ...
            call log_svals(logfile_basename, X, tau, svals, svals_lag, RiccSolver_counter, istep, opts%nsteps)
            ! Check convergence
   ...
   subroutine init_ricc(X, A, B, CT, Qc, Rinv, Tend, tau, exptA, opts, iftrans, options)
      class(abstract_sym_low_rank_state_rdp),  intent(inout) :: X
      !! Low-Rank factors of the solution.
      class(abstract_linop_rdp),               intent(inout) :: A
      !! Linear operator
      ...
      call log_settings(X, Tend, opts)
   end subroutine init_ricc

   !-----------------------
   !-----     PSI     -----
   !-----------------------

   ! subroutine projector_splitting_DLRA_step_riccati_rdp(X, A, B, CT, Qc, Rinv, tau, mode, info, exptA, trans)
   ...
   !    if (time_lightROM()) call lr_timer%start('DLRA_step_riccati_rdp')
   ...
   !    if (time_lightROM()) call lr_timer%stop('DLRA_step_riccati_rdp')
   ! end subroutine projector_splitting_DLRA_step_riccati_rdp
   ...
   ! subroutine rank_adaptive_PS_DLRA_step_riccati_rdp(X, A, B, CT, Qc, Rinv, tau, mode, info, rk_reduction_lock, exptA, trans, tol)
   !    !! Wrapper for projector_splitting_DLRA_step_riccati_rdp adding the logic for rank-adaptivity
   !    class(abstract_sym_low_rank_state_rdp), intent(inout) :: X
   !    !! Low-Rank factors of the solution.
   !    class(abstract_linop_rdp),              intent(inout) :: A
   !    !! Linear operator
   ...
   !       ! run a regular step
   !       call projector_splitting_DLRA_step_riccati_rdp(X, A, B, CT, Qc, Rinv, tau, mode, info, exptA, trans)
   !       ! compute singular values of X%S
   !       Usvd = 0.0_dp; ssvd = 0.0_dp; VTsvd = 0.0_dp
   ...
   !    X%rk = rk - 1      
   ! end subroutine rank_adaptive_PS_DLRA_step_riccati_rdp
   ...
   !    if (time_lightROM()) call lr_timer%start('G_map_riccati_rdp')
   ...
   !    if (time_lightROM()) call lr_timer%stop('G_map_riccati_rdp')
