module LightROM_LyapunovSolvers
   ...
   !
   !=======================================================================================
   subroutine numerical_low_rank_splitting_integrator(X,LTI,Tend,tau,torder,info,exptA)
      !> Low-rank state
   ...
   subroutine M_forward_map(X, LTI, tau, info, exptA)
      !> Low-rank state
      class(abstract_sym_low_rank_state), intent(inout) :: X
      ! LTI system
      class(abstract_lti_system),         intent(in)    :: LTI
      ...
      do i = 1, rk
         call exptA(Uwrk, LTI%A, X%U(i), tau, info)
